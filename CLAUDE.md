...
- **Every PR opens as a draft, no exceptions.** Open the PR as a draft immediately — before any review pass, not just for multi-push work. Sequence is always: **open draft → self-review → fix findings → verify locally → flip to `ready_for_review`.** Never flip to ready until that sequence is done; flipping ready is what should trigger CI, so an unreviewed draft never burns a CI run. CI minutes are a shared, metered resource, same discipline as token budget.
- **Repos whose CI still runs on drafts need fixing, not working around.** `pull_request` triggers in GitHub Actions run on draft PRs by default — opening as draft only saves CI minutes if the workflow explicitly skips drafts (e.g. a job/step `if: github.event.pull_request.draft == false`, or gating on the `ready_for_review` action). If you're touching a repo's CI config and notice it lacks this guard, add it as part of that work.
- **Never use CI itself as the debugging loop.** Don't reach for `workflow_dispatch`, manual re-runs, or repeat pushes to force another CI pass in order to chase a failure (flaky tests, environment-dependent bugs) — reproduce and fix it locally first (project docs/memory usually have a local-equivalent recipe; write one down via `claude-md-management:revise-claude-md` if it's missing). Manual CI triggers are for validating something that genuinely can't be reproduced locally (e.g. a hosted data-plane connection), not for debug-by-rerun.
