...
- **Set the model by the subtask's own complexity, not by inheriting the session's.** `Agent` and `Workflow`'s `agent()` calls both take a `model` param — use it: mechanical, single-file, no-judgment-call work (Haiku-class) shouldn't ride on an Opus session, and genuinely cross-cutting/ambiguous/one-way-door work (Opus-class) shouldn't be quietly done at Sonnet just because that's the session default. When a subtask maps to an issue carrying an Effort value on the shared project, that's the answer — otherwise judge it the same way.
