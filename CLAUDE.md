...
- **Open as draft, verify locally, then flip to ready.** For anything that will take several iterative pushes before it's mergeable, open the PR as a draft immediately and mark it `ready_for_review` only once local verification passes. In repos whose CI is configured to skip draft PRs, this turns N iterative CI runs into one — CI minutes are a shared, metered resource, same discipline as token budget. Where CI doesn't skip drafts, it still costs nothing, so default to it everywhere.
