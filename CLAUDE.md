...
- **Self-review and verify locally *before* opening the PR; then open it ready — not draft.** Do the review pass and the change-scoped checks on the branch first, so the first thing CI ever sees is already self-reviewed and green. That's what keeps CI off unreviewed work — with no draft→ready flip to fumble (the flip confuses more than it saves). CI minutes are a shared, metered resource, same discipline as the token budget; when you touch a repo's CI, keep it cheap and non-stacking (see `rules/ci.md`).
