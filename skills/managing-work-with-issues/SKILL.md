...
**If falling back to the raw `gh` CLI, use `gh api`, never `gh issue list`/`gh pr list`.** Those two commands route through GitHub's GraphQL API, whose 5000/hr quota is shared across every agent and session authenticating as the same token — a concurrent multi-agent dedupe pass can exhaust it in minutes, and the failure (`GraphQL: API rate limit already exceeded`) reads like an auth problem, not a quota one. REST's `core` quota is separate and far less contended. Prefer `issue_list`/`pr_list` (which already take a `fields` param to keep a dedupe pass cheap), or `gh api "repos/OWNER/REPO/issues?state=all&per_page=100&page=N" --jq '.[] | select(.pull_request == null) | "\(.number) \(.state) \(.title)"'` as the REST-based raw-CLI fallback.
