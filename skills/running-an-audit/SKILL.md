...
**2. Verify the guard fires, against the real regression.** A guard that exists and never runs is the most expensive outcome available: it converts an open problem into a false pass. Before trusting any check — existing or newly added — confirm it goes red. Restore the genuine defect to do it: `git checkout <sha-that-had-the-bug> -- <path>`, or `git checkout origin/<trunk> -- <path>` where the defect predates the branch. A hand-written approximation can be strictly *easier* than the real thing — unused where the original was used, shallower where it was nested, synthetic where it interacted with a second file — and passing it proves nothing. If the guard does not fire on your hand-written break, the first hypothesis is that the break is wrong, not the guard. **Commit before you break anything on purpose**, so the revert cannot take unrelated uncommitted work with it.
