...
- **Hand-rolled SVGs that don't sit on the icon set's grid.** A one-off SVG pasted in for a shape the library didn't have — wrong stroke width, wrong viewBox, wrong optical size, hard-coded `fill` that ignores `currentColor` and so never adapts to dark mode, an uncompressed export still carrying the design tool's metadata and clip paths. It reads as janky next to the twelve library icons around it, and the tell is visible before anyone can name it. Every custom SVG matches the set's grid, stroke, and corner radius, uses `currentColor`, is run through an optimizer, and is decorative (`aria-hidden`) or titled.
- **Font weights used without a hierarchy.** Four weights on one screen, semibold and bold both doing "emphasis" with no rule for which, body copy at a weight the typeface wasn't designed for, or a heading distinguished from its body only by weight and not by size or spacing. The design system names the weights it uses and what each one means; a weight appearing outside that set is the finding, and inconsistent hierarchy across two surfaces is a `ux-coherence.md` finding as well.
