...
> **Hooks are the real guardrail here.** `settings.json` runs in `bypassPermissions` mode, where the allow/deny permission system is skipped — but hooks still fire. `guard-bash.sh` hard-blocks `--no-verify`, committing `.env`, force-push to `main`/`master`, and reckless `rm -rf` of root/home/parent paths, while leaving everyday commands (`rm -rf node_modules`, pushing feature branches) untouched.
...
2. Symlinks the `rules/` and `hooks/` directories into `~/.claude/`
3. Symlinks each skill under `skills/` into `~/.claude/skills/` (individually, so plugin/user skills there aren't clobbered)
4. Reports which plugins will auto-install on first `claude` launch
